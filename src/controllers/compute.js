...
.controller('ComputeCtrl', function($scope, $http, $location, Criteria, Place, Tweet, Calc) {
  ...
  // criteria.city.geometry.location

  $http({
    method: 'GET',
    url: 'http://localhost:4730/tweets/' + criteria.query + '/' + criteria.city.geometry.location.lat + ',' + criteria.city.geometry.location.lng + ',' + criteria.radius + 'km'//'http://localhost:4730/tweets/museum/51.507222,-0.1275,50km'
  }).then(function successCallback(response) {
    // this callback will be called asynchronously
    // when the response is available
    console.log(response)

    Tweet.set(response.data)
